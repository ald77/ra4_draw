...
int main(int argc, char *argv[]){
  ...
  vector<unsigned> sigcol({kRed+1,kGreen+2, kAzure-6, kOrange+4});
  ...
  binnames.push_back("SBD, 2b");
  binnames.push_back("HIG, 2b");
  binnames.push_back("SBD, #geq3b");
  binnames.push_back("HIG, #geq3b");
  ...
  legX = 0.5;
  double legW = 0.40, legH = legSingle*sigm.size();
  ...
  for (unsigned ibin(0); ibin<nbins; ibin++){
    graph[ibin] = TGraphAsymmErrors(ratio[0].size(), 
      &(xcoord[ibin][0]), &(ratio[ibin][0]),
      &(xerr[ibin][0]), &(xerr[ibin][0]), 
      &(err_dn[ibin][0]), &(err_up[ibin][0]));
    graph[ibin].SetMarkerStyle(20+ibin); 
    graph[ibin].SetMarkerSize(1.8);
    graph[ibin].SetMarkerColor(sigcol[ibin]);
    graph[ibin].SetLineColor(sigcol[ibin]); 
    graph[ibin].SetLineWidth(2);
    graph[ibin].Draw("p0 same");
